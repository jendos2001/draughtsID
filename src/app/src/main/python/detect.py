...
def getDiff(dfList: list, isVertical: bool, isBlack: bool, prevPos: str) -> str:
    print(prevPos)
    cur = ""
    for elem in dfList:
        if not (elem.name == 'Board').any():
    ...
    if len(diffWhite) == 0 and len(diffBlack) == 0:
        return f"|{cur}"
    elif len(diffWhite) <= 1 and len(diffBlack) == 2:
        if list(diffBlack)[0] in blackPrev:
            return getMoveByPos(list(diffBlack)[0], list(diffBlack)[1]) + f"|W:{cur}"
        else:
            return getMoveByPos(list(diffBlack)[1], list(diffBlack)[0]) + f"|W:{cur}"
    elif len(diffWhite) == 2 and len(diffBlack) <= 1:
        if list(diffWhite)[0] in whitePrev:
            return getMoveByPos(list(diffWhite)[0], list(diffWhite)[1]) + f"|W:{cur}"
        else:
            return getMoveByPos(list(diffWhite)[1], list(diffWhite)[0]) + f"|W:{cur}"

    else:
        return f"|W:{cur}"


def getMoveByPos(pos1: str, pos2: str) -> str:
    if pos1[0] == 'K':
        pos1 = pos1[1:]
    if pos2[0] == 'K':
        pos2 = pos2[1:]
...
def getMove(img, pos, pos2, prevPos) -> str:
    image = Image.open(io.BytesIO(base64.decodebytes(bytes(img, "utf-8"))))
    weights_name = join(dirname(__file__), "best.pt")
    model = torch.hub.load('ultralytics/yolov5', 'custom', weights_name)
    model.conf = 0.6
    results = model(image)
    return getDiff(results.pandas().xyxy, pos == 0, pos2 == 1, prevPos)
